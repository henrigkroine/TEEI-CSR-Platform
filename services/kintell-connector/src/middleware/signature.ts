...
const logger = createServiceLogger('kintell-connector:signature');

/**
 * HMAC-SHA256 signature validation middleware for Kintell webhooks
 *
 * Security features:
 * - Validates X-Kintell-Signature header against computed HMAC-SHA256
 * - Prevents replay attacks with timestamp tolerance (±5 minutes)
 * - Uses timing-safe comparison to prevent timing attacks
 *
 * Required environment variables:
 * - KINTELL_WEBHOOK_SECRET: Shared secret for HMAC computation
 *
 * Expected headers:
 * - X-Kintell-Signature: HMAC-SHA256 signature in format "t=<timestamp>,v1=<signature>"
 * - X-Delivery-Id: Unique delivery ID for idempotency
 */

const WEBHOOK_SECRET = process.env.KINTELL_WEBHOOK_SECRET || 'dev-secret-change-in-production';
...
export async function validateWebhookSignature(
  ...
  const signatureHeader = request.headers['x-kintell-signature'] as string | undefined;
  const deliveryId = request.headers['x-delivery-id'] as string | undefined;

  // Check required headers
  if (!signatureHeader) {
    logger.warn({ url: request.url }, 'Missing X-Kintell-Signature header');
 ...
 * Usage: Add to webhook sender to generate the X-Kintell-Signature header
 */
