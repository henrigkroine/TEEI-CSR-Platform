...
import type { GeneratedReport, PDFExportOptions, ReportSection } from '../types';
 ...
 * Generate cover page
 */
function generateCoverPage(report: GeneratedReport, options: TemplateOptions): string {
 ...
 */
function generateTableOfContents(report: GeneratedReport): string {
 ...
 */
function generateSections(report: GeneratedReport, options: TemplateOptions): string {
  if (!report.sections || report.sections.length === 0) {
 ...
 */
function generateSection(section: ReportSection, options: TemplateOptions): string {
 ...
 */
function processCitations(narrative: string, citations: any[]): string {
 ...
 * Generate chart HTML
 */
function generateChartHTML(chart: any, base64Image: string): string {
  return `
 ...
 */
function generateCitationsSection(report: GeneratedReport): string {
  ...
  const uniqueCitations = Array.from(
    new Map(allCitations.map((c) => [c.evidenceId, c])).values()
  );
