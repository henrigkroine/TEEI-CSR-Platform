import Fastify from 'fastify';
import { createServiceLogger } from '@teei/shared-utils';
import { getEventBus } from '@teei/shared-utils';
import { flagsRoutes } from './routes/flags.js';
import { milestonesRoutes } from './routes/milestones.js';
import { rulesRoutes } from './routes/rules.js';
import { setupSubscribers } from './subscribers/index.js';

const logger = createServiceLogger('journey-engine');
const PORT = parseInt(process.env.PORT_JOURNEY_ENGINE || '3024');

async function start() {
  const app = Fastify({
    logger: logger as any,
  });

  // Health check
  app.get('/health', async () => {
    return {
      status: 'ok',
      service: 'journey-engine',
      timestamp: new Date().toISOString(),
    };
  });

  // Register routes
  app.register(flagsRoutes, { prefix: '/journey' });
  app.register(milestonesRoutes, { prefix: '/journey' });
  app.register(rulesRoutes, { prefix: '/journey' });

  // Connect to event bus and setup subscribers
  const eventBus = getEventBus();
  await eventBus.connect();
  await setupSubscribers(eventBus);

  // Start server
  try {
    await app.listen({ port: PORT, host: '0.0.0.0' });
    logger.info(`Journey Engine Service running on port ${PORT}`);
  } catch (err) {
    logger.error(err);
    process.exit(1);
  }

  // Graceful shutdown
  const shutdown = async () => {
    logger.info('Shutting down...');
    await eventBus.disconnect();
    await app.close();
    process.exit(0);
  };

  process.on('SIGINT', shutdown);
  process.on('SIGTERM', shutdown);
}

start();
