 ...
 */
export async function getSDGDistribution(startDate, endDate) {
    ...
    for (const [sdg, count] of sdgCounts.entries()) {
        const reference = sdgMappings.sdgReference[sdg.toString()];
        if (reference) {
            distribution.push({
                sdg,
                name: reference.name,
 ...
 */
export async function getEventsBySDG(filter) {
 ...
 */
export async function getSDGCoverageReport(startDate, endDate) {
    ...
    logger.info({ period, totalEvents, coveredSDGs: coveredSDGs.length }, 'SDG coverage report generated');
    return report;
}
/**
 ...
 */
export async function getQuarterlySDGReport(year, quarter) {
    const quarterStarts = {
        1: new Date(year, 0, 1),
        2: new Date(year, 3, 1),
        3: new Date(year, 6, 1),
 ...
 */
export async function getSDGBreakdownByEventType(sdg, startDate, endDate) {
...
//# sourceMappingURL=sdg-queries.js.map