 ...
 */
export const eventDeduplication = pgTable('event_deduplication', {
    ...
    eventConsumerUnique: uniqueIndex('event_consumer_unique_idx').on(table.eventId, table.consumerId),
    // Index for querying by event type and consumer
    eventTypeConsumerIdx: index('event_type_consumer_idx').on(table.eventType, table.consumerId),
 ...
 */
export const webhookDeduplication = pgTable('webhook_deduplication', {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    deliverySourceUnique: uniqueIndex('delivery_source_unique_idx').on(table.deliveryId, table.webhookSource),
    // Index for querying by webhook source and type
    sourceTypeIdx: index('webhook_source_type_idx').on(table.webhookSource, table.webhookType),
 ...
 */
export const apiRequestDeduplication = pgTable('api_request_deduplication', {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    idempotencyKeyUnique: uniqueIndex('idempotency_key_unique_idx').on(table.idempotencyKey),
    // Index for user-scoped queries
    userIdempotencyIdx: index('user_idempotency_idx').on(table.userId, table.idempotencyKey),
