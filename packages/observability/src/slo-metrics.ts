...
export class SLOMetrics {
  ...
  private readonly sloCurrentValueGauge: Gauge<string>;
  private readonly sloTargetValueGauge: Gauge<string>;

  // Gauges for error budgets
  private readonly errorBudgetRemainingGauge: Gauge<string>;
  private readonly errorBudgetConsumedGauge: Gauge<string>;
  private readonly errorBudgetBurnRateGauge: Gauge<string>;

  // Counter for SLO violations
  private readonly sloViolationsCounter: Counter<string>;

  // Histogram for error budget consumption rate
  private readonly errorBudgetConsumptionHistogram: Histogram<string>;
  ...
  updateFromSLOStatus(status: SLOStatus): void {
    ...
    this.sloComplianceGauge.set(
      { slo_name: labels.slo_name, slo_type: labels.slo_type, slo_window: labels.slo_window },
      status.compliance ? 1 : 0
    );
    ...
    this.errorBudgetRemainingGauge.set(
      { slo_name: labels.slo_name, slo_window: labels.slo_window },
      status.errorBudgetRemaining
    );

    this.errorBudgetConsumedGauge.set(
      { slo_name: labels.slo_name, slo_window: labels.slo_window },
      status.errorBudgetConsumed
    );

    this.errorBudgetBurnRateGauge.set(
      { slo_name: labels.slo_name, slo_window: labels.slo_window },
      status.burnRate
    );

    // Record consumption rate in histogram
    this.errorBudgetConsumptionHistogram.observe(
      { slo_name: labels.slo_name, slo_window: labels.slo_window },
      status.burnRate
    );
 ...
 */
let globalSLOMetrics: SLOMetrics | null = null;

/**
 * Get or create global SLO metrics instance
 */
export function getSLOMetrics(registry?: Registry): SLOMetrics {
  if (!globalSLOMetrics) {
 ...
 */
export function registerSLOMetricsEndpoint(
  fastify: any,
  ...
  // Metrics endpoint
  fastify.get('/metrics/slo', async (_request: any, reply: any) => {
  ...
  fastify.get('/api/slo/status', async (_request: any, reply: any) => {
    const statuses = sloManager.getAllSLOStatuses();
  ...
  fastify.get('/api/slo/status/:sloKey', async (request: any, reply: any) => {
