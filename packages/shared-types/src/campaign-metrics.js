...
}).refine((data) => data.budgetSpent <= data.budgetAllocated, {
    message: "Budget spent cannot exceed budget allocated",
    path: ["budgetSpent"],
}).refine((data) => data.volunteersCurrent <= data.volunteersTarget || data.volunteersTarget === 0, {
 ...
 */
export function isSnapshotNearCapacity(snapshot, threshold = 0.8) {
 ...
 */
export function isSnapshotOverCapacity(snapshot) {
    const utilization = parseFloat(snapshot.volunteersUtilization);
 ...
 */
export function hasSnapshotBudgetWarning(snapshot, threshold = 0.9) {
 ...
 */
export function hasSnapshotImpactMetrics(snapshot) {
    return snapshot.sroiScore !== null || snapshot.averageVISScore !== null;
}
/**
 * Check if snapshot has alerts
 */
export function hasSnapshotAlerts(snapshot) {
    return snapshot.fullSnapshot.alerts !== undefined && snapshot.fullSnapshot.alerts.length > 0;
}
/**
 * Get critical alerts from snapshot
 */
export function getCriticalAlerts(snapshot) {
    if (!snapshot.fullSnapshot.alerts)
        return [];
    return snapshot.fullSnapshot.alerts.filter(alert => alert.type === 'capacity_critical' || alert.type === 'performance_low');
}
//# sourceMappingURL=campaign-metrics.js.map